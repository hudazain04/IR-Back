...
class SearchService:
    ...
    def load_word2vec_assets(self, dataset_name: str):
        ...
        if not self.word2vec_loaded:
            ...
            try:
                self.w2v_model: Word2Vec = joblib.load(os.path.join(base_dir, "w2v_model.joblib"))
                self.w2v_matrix = joblib.load(os.path.join(base_dir, "w2v_matrix.joblib"))
                self.doc_ids = joblib.load(os.path.join(base_dir, "doc_ids.joblib"))
                self.doc_texts = joblib.load(os.path.join(base_dir, "doc_texts.joblib"))
                self.word2vec_loaded = True
